...
***************************************************************************
  ...
  - Padding % → 0
  - Token Vision → off
  - Fog Exploration → None (0=None, 1=Individual, 2=Shared)
...
// Set Fog Exploration mode (v14). On v13 anything non-zero counts as enabled.
//  0 = NONE (disabled)
//  1 = INDIVIDUAL (each user has their own fog of war)
//  2 = SHARED (fog of war shared among all users)
const CONFIG_FOG_EXPLORATION = 0;
			...
			<div class="section">
				...
				<table class="settings-table">
					<tbody>
						<tr>
							<td>
								<label><input type="checkbox" name="applyBackgroundColor" checked> Background Color</label>
								<label title="v14 only: apply Background Color to every level, not just the default"><input type="checkbox" name="bgAllLevels"> all levels</label>
							</td>
			...
			buttons: [
				{
					...
					callback: async (event, button, dialog) => {
						...
						const bgAllLevels = form.elements.bgAllLevels?.checked;
						if (!Object.values(apply).some(Boolean)) return ui.notifications?.warn("No settings were chosen to apply.");

						// v14 moved background color onto the Scene's default Level, and fog/light/darkness
						// into fog.mode + environment.*; v13 keeps the old top-level fields.
						...
						for (const id of selectedIds) {
							...
							if (isV14) {
								// Background color lives on the Level document(s); all levels if requested, else default
								if (apply.background) {
									const levels = bgAllLevels
										? (scn?.levels?.contents ?? [])
										: [scn?.firstLevel ?? scn?.levels?.contents?.[0]].filter(Boolean);
									const levelUpdates = levels.map(l => ({ _id: l.id, "background.color": CONFIG_BACKGROUND_COLOR }));
									if (levelUpdates.length) u.levels = levelUpdates;
								}
								if (apply.fog) u["fog.mode"] = CONFIG_FOG_EXPLORATION;
								if (apply.globalLight) u["environment.globalLight.enabled"] = CONFIG_GLOBAL_LIGHT;
								if (apply.darkness) {
									// Clear the lock, else _preUpdate drops the darknessLevel change on locked scenes
									u["environment.darknessLock"] = false;
									u["environment.darknessLevel"] = CONFIG_DARKNESS;
								}
							} else {
								if (apply.background) u["backgroundColor"] = CONFIG_BACKGROUND_COLOR;
								if (apply.fog) u["fogExploration"] = CONFIG_FOG_EXPLORATION !== 0;
